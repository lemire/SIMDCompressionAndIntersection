...
public:
     ...
     * by ID).
     */
    size_t getSizeInInts(uint32_t postId) {
        return mapuncompsizes[postId];
    }


    /**
     * Load an array (data) of length "length" as the posting list corresponding to id postid
     * into the data structure. The data will be either converted to a bitmap or compressed.
     */
    ...
    size_t sizeOfRecoveryBufferInWords() const {
        return recovbuffer.size();
    }
private:

    // load as either a bitmap or a compressed short list
    size_t loadOptimized(const uint32_t postid, const uint32_t *data, const uint32_t length) {
        if (mapuncompsizes.find(postid) != mapuncompsizes.end()) return 0;
        vector<uint32_t> *compressedbuffer = new vector<uint32_t>(length + 1024);
        size_t outlength = compressedbuffer->size();
        vector<uint32_t> tmp(data, data + length); // use the buffer because some codecs modify the input
        codec.encodeArray(tmp.data(), length, compressedbuffer->data(), outlength);
        if (outlength * sizeof(uint32_t) < BoolArray::sizeInBytes(mMaxId)) { // we are good
            if (recovbuffer.size() < length) recovbuffer.resize(length);
     ...
     */
    size_t loadAsBitmap(const uint32_t postid, const uint32_t *data, const uint32_t length) {
        if (bitmapmap.find(postid) != bitmapmap.end()) return 0;
     ...
     * Do not call this directly, call load() instead.
     */
    size_t loadAsShortArray(const uint32_t postid, const uint32_t *data, const uint32_t length) {
        if (shortlistmap.find(postid) != shortlistmap.end()) return 0;
        if (recovbuffer.size() < length) recovbuffer.resize(length);
        vector<uint32_t> *compressedbuffer = new vector<uint32_t>(length + 1024);
        size_t outlength = compressedbuffer->size();
        for (size_t i = 0; i < length; ++i) // use the buffer because some codecs modify the input
            recovbuffer[i] = data[i];
        codec.encodeArray(recovbuffer.data(), length, compressedbuffer->data(), outlength);
        compressedbuffer->resize(outlength);
        compressedbuffer->shrink_to_fit();
        shortlistmap[postid] = shared_ptr<vector<uint32_t>>(compressedbuffer);
        mapuncompsizes[postid] = length;
...
public:
    ...
    size_t intersect(const vector<uint32_t> &ids, uint32_t *out,
        ...
        if (shortlists.empty()) {
            ...
            assert(sizeout == shortlists[0].first);
            // we have to make a copy because by convention the output is not directly from the index
            ...
            for (uint32_t i = 1; (sizeout > 0) && (i < shortlists.size()); ++i) {
                ...
                sizeout = Inter(out, sizeout, shortlists[i].second->data(), shortlists[i].second->size(), out);
            }
            size_t pos = 0;
            for (uint32_t i = 0; (sizeout > 0) && (i < bitmaps.size()); ++i) {
                unpackVolume += bitmaps[i].first;
...
private:
    ...
    size_t loadAsShortArray(const uint32_t postid, const uint32_t *data, const uint32_t length) {
        if (shortlistmap.find(postid) != shortlistmap.end()) return 0;
        mapuncompsizes[postid] = length;
        vector<uint32_t> *compressedbuffer = new vector<uint32_t>(data, data + length);
        shortlistmap[postid] = shared_ptr<vector<uint32_t>>(compressedbuffer);
        return compressedbuffer->size();
    }
...
public:
    ...
    SkippingHybM2(uint32_t MaxId,
                  uint32_t th = 32, uint32_t BS = 8) :
        bitmapmap(), shortlistmap(), mapuncompsizes(), mMaxId(MaxId),
        threshold(th),
        BlockSizeLog(BS) {
    }
