...
public:
	...
	void encodeArray(uint32_t *in, const size_t length,
	...
	virtual std::string name() const {
		return std::string("VarIntG8IU") + ((delta==1)?"scalardelta":((delta==2)?"delta":""));
	}

	int encodeBlock(const uint32_t*& src, size_t& srclength,
			unsigned char*& dest, size_t& dstlength, uint32_t & prev) {
		...
		while (srclength > 0) {
			...
			int byteNeeded = delta ? getNumByteNeeded(*temp - prev) : getNumByteNeeded(*temp);
