...
struct SIMDBitPackingHelpers {
    ...
    static void unpack(const uint32_t * in, size_t Qty, uint32_t * out,  const uint32_t bit ) {
        if (Qty % SIMDBlockSize) {
            throw std::logic_error("Incorrect # of entries.");
        }
        if (Qty % SIMDBlockSize) {
        ...
        for(size_t k = 0; k < Qty/SIMDBlockSize; ++k) {
            simdunpack(reinterpret_cast<const __m128i*>(in+SIMDBlockSize*k*bit/32),out+k*SIMDBlockSize,bit);
            if(bit<32) {
                initoffset = SIMDDeltaProcessor<DeltaHelper, SIMDBlockSize>::runPrefixSum(initoffset,out+k*SIMDBlockSize);
            }
        }
    }
    ...
    static void ipack(const uint32_t * in, const size_t Qty, uint32_t * _out,  const uint32_t bit ) {
        if (Qty % SIMDBlockSize) {
            throw std::logic_error("Incorrect # of entries.");
        }
    ...
    static void ipackwithoutmask(const uint32_t * in, const size_t Qty, uint32_t * _out,  const uint32_t bit ) {
        if (Qty % SIMDBlockSize) {
            throw std::logic_error("Incorrect # of entries.");
        }
        __m128i *out = reinterpret_cast<__m128i*>(_out);
    ...
    static void iunpack(const uint32_t * _in, size_t  Qty, uint32_t * out,  const uint32_t bit ) {
        if (Qty % SIMDBlockSize) {
            throw std::logic_error("Incorrect # of entries.");
        }
        ...
        for(size_t k = 0; k < Qty/SIMDBlockSize; ++k) {
            initoffset = SIMDiunpack<DeltaHelper>(initoffset,in+k*bit,out+k*SIMDBlockSize,bit);
        }
    }

    // this is not expected to be useful, only for benchmarking
    static void ipatchedunpack(const uint32_t * _in, size_t  Qty, uint32_t * out,  const uint32_t bit ) {
        if (Qty % SIMDBlockSize) {
            throw std::logic_error("Incorrect # of entries.");
        }
        const __m128i *in = reinterpret_cast<const __m128i*>(_in);
        ...
        for(size_t k = 0; k < Qty/SIMDBlockSize; ++k) {
            initoffset = SIMDipatchedunpack<DeltaHelper>(initoffset,in+k*bit,out+k*SIMDBlockSize,reinterpret_cast<const __m128i*>(out+k*SIMDBlockSize),bit);
        }
    }
    ...
    static void CheckMaxDiff(const std::vector<uint32_t>& refdata, unsigned bit) {
        for(size_t i = 4; i < refdata.size(); ++i ) {
        	if(gccbits(refdata[i]-refdata[i-4])>bit) throw std::runtime_error("bug");
        }
    }
};
