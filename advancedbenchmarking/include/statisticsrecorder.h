...
public:
    ...
    double averageTimeInMS() const {
        if(timesinmicros.size() == 0) return 0;
        return static_cast<double>(std::accumulate(timesinmicros.begin(),timesinmicros.end(),0)) * 0.001 /
                static_cast<double>(timesinmicros.size());
    }
    ...
    double medianTimeInMS() const {
        ...
        return static_cast<double>(buffer[buffer.size()/2]) * 0.001;// not *exactly* the median but close enough
    }
    ...
    double ninetypercentileTimeInMS() const {
        ...
        vector<uint64_t> buffer(timesinmicros);
        sort(buffer.begin(),buffer.end());
        return static_cast<double>(buffer[static_cast<size_t>(round(buffer.size()*0.9))]) * 0.001;// not *exactly* the 90 perc. but close enough
    }
