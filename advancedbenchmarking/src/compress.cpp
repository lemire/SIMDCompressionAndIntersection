...
int main(int argc, char **argv) {
    ...
    try {
        ...
        if (schemeptr.get() == NULL) return -1;
        ...
        cout << "Compressing content from " << ifilename << " to " << ofilename << " using " << scheme << endl;
        ...
        while (reader.loadIntegers(buffer)) {
            ...
            schemeptr->encodeArray(buffer.data(), buffer.size(), obuffer.data(), outsize);
        ...
        if (volume > 0)
            cout << "Bits per int : " << (8.0 * static_cast<double>(volumeout)) / static_cast<double>(volume) << endl;
        if (ti > 0) {
