...
int main(int argc, char **argv) {
    ...
    cout << " generating " << (cluster ? "cluster" : "uniform") <<
         " " << howmany << " arrays of length = " << N <<
         " over " << range << " bits to file " << ofilename << endl;
    WallClockTimer z;
    for (size_t i = 0; i < howmany; ++i) {
        vector<uint32_t> array = cluster ? cdg.generate(N, Max) : udg.generate(N, Max);
        assert(array.size() == N);
