...
void printusage(char *prog) {
    ...
    cout << "example: likwid -m -C 1 -g BRANCH " << prog << " input.bin scheme1 scheme2 ..." << endl;
	cout
...
struct stats {
	stats() :
			VolumeDecoded(0), BytesDecoded(0), TimeDecoding(0), NumberOfArrays(
					0) {
	}
...
void simpleProcessArray(
		map<shared_ptr<IntegerCODEC>,stats> & mystats,
		vector<vector<uint32_t>> & mbuffer, vector<vector<uint32_t>>& mobuffer, uint32_t & bogus) {
	...
	for (auto p = mystats.begin(); p != mystats.end(); ++p) {
		...
		for(size_t K = 0; K < mbuffer.size(); ++K) {
			...
			if( l1buf.size() < buffer.size() ) l1buf.resize(buffer.size());
			vector<uint32_t> & obuffer = mobuffer[K];
			vector<uint32_t> bufcopy(buffer); // we make a copy so that no funny business can happen
...
void processArray(
	...
	for (auto p = mystats.begin(); p != mystats.end(); ++p) {
		...
		for(size_t K = 0; K < mbuffer.size(); ++K) {
			...
			s.VolumeDecoded += buffer.size();
			s.BytesDecoded += obuffer.size() * sizeof(uint32_t);
			// we first do it without recording results to make sure no funny buffering issues
...
int main(int argc, char **argv) {
	vector<uint32_t> blocksizes = {32,64,128,256,512,1024,2048, 4096, 8192, 16384, 32768, 65536,1048576};
	...
	try {
		...
		for (auto p = mystats.begin(); p != mystats.end(); ++p) {
			...
			shared_ptr<IntegerCODEC> codec = p->first.second;
			stats & s = p->second;
			if(s.NumberOfArrays != numberofarrays) throw runtime_error("bug");
			string codecname = CODECFactory::getName(*codec);
			cout<<left<<setw(10)<<blocksize<<setw(30)<<left<<codecname
			<<setw(20)<<left<<round(static_cast<double>(s.VolumeDecoded)/static_cast<double>(s.TimeDecoding))
			<<setw(20)<<left<<static_cast<double>(s.BytesDecoded)*8.0/static_cast<double>(s.VolumeDecoded)<<endl;
		}
