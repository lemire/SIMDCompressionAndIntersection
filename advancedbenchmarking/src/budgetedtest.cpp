...
public:
    ...
    */
    ...
    void test(BudgetedPostingCollector& uncompPosts, const vector<vector<uint32_t>>& allPostIds) {
        unordered_map<uint32_t,vector<uint32_t> > compPostings; // we use a hash table to store the postings, ought to fit in RAM
    ...
    void skippingtest(int SkipLog, BudgetedPostingCollector& uncompPosts,
            const vector<vector<uint32_t>>& allPostIds) {
        unordered_map<uint32_t,shared_ptr< Skipping> > compPostings; // we use a hash table to store the postings, ought to fit in RAM
        size_t MaxPostingSize(0);
        ...
        for(vector<uint32_t> qids : allPostIds) {
            ...
            if(sizeids.size() == 1) {
                ...
                for(size_t k = 2; (intersize>0) && (k < sizeids.size()); ++k) {
                    ...
                    intersize = compPostings[sizeids[k].second]->intersect(intersection_result.data(),intersize,intersection_result.data());
                }
            }
    ...
    void printNumbers(bool detailed) const {
        ...
        cout<<"# max recovery buffer = "<<setprecision(4)<<(static_cast<double>(MaxRecoveryBuffer*sizeof(uint32_t))/(1024.0*1024.0))<<"MB"<<endl;
