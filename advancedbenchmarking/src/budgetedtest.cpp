...
void printusage(char *prog) {
    cout << "Usage: " << prog <<
         " <uncompressed postings in the flat format> <converted log file> -b <memory budget> -s <codec>  -l <max # of log lines to process> -i <intersection function> -o "
         << endl;
    cout << "       -s   specify compression scheme ( " ;
    for (string s : CODECFactory::allNames()) cout << s << " ";
    cout << ")" << endl;
    cout << " (If no compression scheme is specified, we fall back on uncompressed postings.)" << endl;
    cout << "       -i   specify intersection function ( " ;
    for (string s : IntersectionFactory::allNames()) cout << s << " ";
    cout << ")" << endl;
    cout << "       -o   include one-word queries (default is do not include them)" << endl;
    cout << "       -q   quiet mode, runs entire test before outputting results (default is verbose)" << endl;
    cout << "       -l   allow you to specify a limit on the number of queries" << endl;
    cout << "       -p   partition the postings during compression and decompression (param: number of partitions)" << endl;
    cout << "       -k   to test Skipping (must provide a gap size param between 1 and 31)" << endl;
    cout << "       -B   to activate bitmap mode (recommended values are in the range 8 to  32), when set to 0 in conjunction with compression, it means 'automatic' "
         << endl;
...
class TestHelper {
public:
    TestHelper(IntegerCODEC &scheme,
               intersectionfunction  Inter) :
        scheme(scheme),
        CompressedSizeDuringPacking(0),
        packVolume(0), unpackVolume(0),
        packTime(0), unpackTime(0), interTime(0),
        Inter(Inter),  CoarsePackTime(0),
    ...
    */
    ...
    /**
     * This is the main testing function. Posting lists are compressed, then uncompressed to RAM
     * and intersected.
     */
    void test(BudgetedPostingCollector &uncompPosts, const vector<vector<uint32_t>> &allPostIds) {
        unordered_map<uint32_t, vector<uint32_t>>
                                               compPostings; // we use a hash table to store the postings, ought to fit in RAM
        ...
        for (const vector<uint32_t> &qids : allPostIds) {
            for (uint32_t id : qids) {
                if (compPostings.find(id) != compPostings.end()) // these hits could potentially slow us down a bit
                    continue; // was compressed earlier
                vector<uint32_t> &onePost = uncompPosts.getOnePost(id);
                size_t qty = onePost.size();
                vector <uint32_t> compressedBuffer(qty); // this is potential a bit expensive: memory allocation
                if (MaxPostingSize < qty) {
                    MaxPostingSize = qty;
                }
                if (modifiesinput) // if true there is a copy which might be expensive
                    dirtyCopy = onePost; // some schemes might modify the input, hence it is necessary to make a copy in general
                ...
                compPostings.emplace(id, compressedBuffer);// with some luck, compressedBuffer is *moved* to the container, not copied
            }
        }
        CoarsePackTime += static_cast<double>(coarsez.split());

        vector <uint32_t> recoveryBuffer(MaxPostingSize + 1024);
        if (MaxRecoveryBuffer < recoveryBuffer.size()) MaxRecoveryBuffer = recoveryBuffer.size();
         ...
         * to the original (duh!)
         */
        for (unordered_map<uint32_t, vector<uint32_t>>::iterator i = compPostings.begin() ; i != compPostings.end(); ++i) {
            ...
            scheme.decodeArray(compressed.data(), compressed.size(),
                               recbuffer.data(), recoveredsize);
            ...
            if (recbuffer != uncompressed) {
                ...
                for (size_t i = 0; i < uncompressed.size() ; ++i) {
                    if (uncompressed[i] != recbuffer[i]) {
                        cout << " i = " << i << " expected " << uncompressed[i] << " but got " << recbuffer[i] << endl;
                        display--;
                        if (display == 0) break;
                    }
                }
        ...
        */
        for (const vector<uint32_t> &qids : allPostIds) {
            ...
            for (uint32_t id : qids) {
                ...
                queryCompPost.emplace_back(make_pair(onePost.size(),  & compPostings[id]));
            }
            ...
            scheme.decodeArray(queryCompPost.front().second->data(), queryCompPost.front().second->size(),
            ...
            for (size_t i = 1; (intersectioncardinality > 0) && (i < queryCompPost.size()); ++i) {
                ...
                scheme.decodeArray(queryCompPost[i].second->data(), queryCompPost[i].second->size(),
                                   recoveryBuffer.data(), recoveredsize);
                assert(recoveredsize <= recoveryBuffer.size());
                intersectioncardinality = Inter(intersection_result.data(), intersectioncardinality,
            ...
            if (trueintersection.size() != intersectioncardinality) {
                ...
                for (auto v : queryCompPost)
                    cout << v.first << endl;
                throw runtime_error("Intersection bug with write-back: you don't even have the correct cardinality.");
            }
            if (!equal(trueintersection.begin(), trueintersection.end(), intersection_result.begin())) {
                cout << "intersectioncardinality=" << intersectioncardinality << endl;
                cout << "number of lists=" << qids.size() << endl;
                for (auto v : queryCompPost)
                    cout << v.first << endl;
                throw runtime_error("you have a bug with intersections using write-backs!");
            }
        }
        /**
         * End of testing
    ...
    void splittedtest(int NumberOfPartitions,
        ...
        pair <uint32_t, uint32_t> range = uncompPosts.findDocumentIDRange(
                                              allPostIds);
        ...
        for (const vector<uint32_t> &qids : allPostIds) {
            for (uint32_t id : qids) {
                ...
                for (int part = 0; part < NumberOfPartitions; ++part) {
                    ...
                    if (thissize > maxSize[part]) maxSize[part] = thissize;
                    i = j;
                }
            }
        }
        ...
        for (const vector<uint32_t> &qids : allPostIds) {
            for (uint32_t id : qids) {
                ...
                for (int part = 0; (i != onePost.end()) && (part < NumberOfPartitions); ++part) {
                    ...
                    if (thissize != 0) {
                        ...
                        size_t nvalue = compressedBuffer.size();
                        vector<uint32_t> dirtyCopy(i, j); // we make a copy because (1) some schemes modify input (2) we need 128-bit alignment
    ...
    void skippingtest(int SkipLog, BudgetedPostingCollector &uncompPosts,
                      const vector<vector<uint32_t>> &allPostIds) {
        unordered_map<uint32_t, shared_ptr<Skipping>>
                compPostings; // we use a hash table to store the postings, ought to fit in RAM
        size_t MaxPostingSize(0);
        ...
        for (const vector<uint32_t> &qids : allPostIds) {
            for (uint32_t id : qids) {
                ...
                vector<uint32_t> &onePost = uncompPosts.getOnePost(id);
                z.reset();
                compPostings.emplace(id , shared_ptr<Skipping>(new Skipping(SkipLog, onePost.data(), onePost.size())));
                packTime += static_cast<double>(z.split());
                size_t qty = onePost.size();
                if (MaxPostingSize < qty) {
                    MaxPostingSize = qty;
                ...
                packVolume += qty;
                CompressedSizeDuringPacking += (compPostings[id]->storageInBytes() + sizeof(uint32_t) - 1) / sizeof(uint32_t);
            }
        }
        CoarsePackTime += static_cast<double>(coarsez.split());
        vector <uint32_t> recoveryBuffer(MaxPostingSize + 1024);
        ...
        for (vector<uint32_t> qids : allPostIds) {
            if (qids.empty()) continue; // odd but could happen?
            SR.prepareQuery();
            ...
            size_t intersize;
            if (sizeids.size() == 1) {
                intersize = compPostings[sizeids.front().second]->decompress(intersection_result.data());
                unpackVolume += intersize;
            } else {
                assert(compPostings.size() >= 2);
                intersize = compPostings[sizeids[0].second]->intersect(*compPostings[sizeids[1].second], intersection_result.data());
                ...
                for (size_t k = 2; (intersize > 0) && (k < sizeids.size()); ++k) {
                    ...
                    intersize = compPostings[sizeids[k].second]->intersect(intersection_result.data(), intersize,
    ...
    void bitmaptestnosplit(uint32_t th, BudgetedPostingCollector &uncompPosts,
        ...
        for (const vector<uint32_t> &qids : allPostIds) {
            for (uint32_t id : qids) {
                if (hybrid.hasBeenLoaded(id)) continue;
                vector<uint32_t> &onePost = uncompPosts.getOnePost(id);
                z.reset();
                CompressedSizeDuringPacking += hybrid.load(id, onePost.data(), onePost.size());
                packTime += static_cast<double>(z.split());
                size_t qty = onePost.size();
                packVolume += qty;
                if (MaxPostingSize < qty) {
        ...
        for (vector<uint32_t> qids : allPostIds) {
            ...
            vector<uint32_t> trueintersection = uncompPosts.computeIntersection(qids, onesidedgallopingintersection);
            if (trueintersection.size() != sizeout) throw runtime_error("not even same cardinality");
            for (uint32_t k = 0; k < sizeout; ++k)
                if (trueintersection[k] != intersection_result[k])  throw runtime_error("intersection bug");
        }
        ...
        for (vector<uint32_t> qids : allPostIds) {
            SR.prepareQuery();
            size_t sizeout = intersection_result.size();
            unpackVolume += hybrid.intersect(qids, intersection_result.data(), sizeout);
    ...
    void bitmapskippingtest(uint32_t BS, uint32_t th, BudgetedPostingCollector &uncompPosts,
                            const vector<vector<uint32_t>> &allPostIds) {
        uint32_t MaxId = uncompPosts.findDocumentIDRange(allPostIds).second;
        size_t MaxPostingSize(0);
    ...
    void uncompressedbitmaptest(uint32_t th, BudgetedPostingCollector &uncompPosts,
                                const vector<vector<uint32_t>> &allPostIds) {
        uint32_t MaxId = uncompPosts.findDocumentIDRange(allPostIds).second;
        size_t MaxPostingSize(0);
    ...
    void testUncompressed(BudgetedPostingCollector &uncompPosts,
        ...
        for (vector<uint32_t> qids : allPostIds) {
            ...
            sort(sizeids.begin(), sizeids.end());
            vector<uint32_t> *answer = & uncompPosts.getOnePost(sizeids.front().second);
            ...
            for (size_t k = 1; (intersize > 0) && (k < sizeids.size()); ++k) {
                ...
                intersize = Inter(answer->data(), intersize,
                                  nextone.data(), nextone.size(), inter.data());
                answer = & inter;
            }
            SR.endQuery(intersize, uncompPosts, qids);
        }
    ...
    void printNumbers(bool detailed) const {
        cout << endl;
        if (! detailed) {
            cout << "#  coarse total speed (mis) + avg (ms/query) + median (ms/query) + 90perc (ms/query)"
                 << endl;
            cout << setw(10) << setprecision(4)
                 << static_cast<double>(unpackVolume)
                 / (CoarseUnpackInterTime) ;
            cout << setw(10) << setprecision(4)
                 << SR.averageTimeInMS();
            cout << setw(10) << setprecision(4)
                 << SR.medianTimeInMS();
            cout << setw(10) << setprecision(4)
                 << SR.ninetypercentileTimeInMS() << endl;

            return;
        }
        cout << "# max recovery buffer = " << setprecision(4) << (static_cast<double>(MaxRecoveryBuffer * sizeof(uint32_t)) /
                (1024.0 * 1024.0)) << "MB" << endl;

        if (packVolume > 0) {
            cout << "# compression: number of integers + bits/int  + speed (mis) + coarse speed (mis) " << endl;
            cout << setw(20) << packVolume << setw(20) << setprecision(4) <<
                 static_cast<double>(CompressedSizeDuringPacking * sizeof(uint32_t)) * 8.0 / static_cast<double>(packVolume)
                 << setw(20) << setprecision(4) << static_cast<double>(packVolume) / packTime
                 << setw(20) << setprecision(4) << static_cast<double>(packVolume) / CoarsePackTime
                 << endl;
        };
        if (unpackVolume > 0) {
            cout
                    << "# decompression: number of integers  + ";
            if (unpackTime > 0)
            ...
            cout << "avg (ms/query)  + median (ms/query) + 90perc (ms/query)"
                 << endl;
...
private:
    ...
    const bool                  modifiesinput;// whether codec modifies the input during compression
...
int main(int argc, char **argv) {
    ...
    size_t memBudget  = 1024ULL * 1024 * 1024 * 4; // 4GB seems like a better default than 16GB
    ...
    while ((c = getopt(argc, argv, "i:os:b:hl:p:qk:B:d")) != -1) {
        ...
        case 'i':
            ...
            if (! IntersectionFactory::valid(InterName)) {
                cerr << "I don't recognize the intersection scheme '" << InterName << "' " << endl;
                printusage(argv[0]);
                return -1;
            }
            break;
    ...
    if (!CODECFactory::valid(scheme)) {
        cout << "Compression scheme " << scheme << " is unknown. Try one of these: " << endl;
        for (string n : CODECFactory::allNames())
            cout << n << endl;
        return -1;
    }
    if ((SkipLog > 0) && (useCompression)) {
        cout << "conflicting options. You cannot mix skipping and compression." << endl;
        printusage(argv[0]);
        return -1;
    }
    const bool modifiesinput = CODECFactory::modifiesInputDuringCompression(*CODECFactory::getFromName(scheme));
    ...
    try {
        ...
        TestHelper testBed(*CODECFactory::getFromName(scheme), IntersectionFactory::getFromName(InterName));
        ...
        if (! quiet) cout << "# After each block, we output the *cumulative* results. Expect convergence. " << endl;
        string line;
        line.reserve(1024);

        vector<vector<uint32_t>>  allPostIds; // this a buffer where queries are stored.
        ...
        for (; (static_cast<size_t>(lineQty - skippedQty) < maxLinesToProcess)  && logFile
            ...
            if (!uncompPosts.loadPostings(oneQueryPostIds)) {// we couldn't load them all in
                if (SkipLog != 0) {
                    if (th >= 0) {
                ...
                if (! quiet) cout << "# queries processed so far: " << (lineQty - skippedQty) << endl;
                if (! quiet) testBed.printNumbers(detailedDisplay);
                uncompPosts.clear();
                allPostIds.clear();
                assert(uncompPosts.getMemUsed() == 0);
                if (!uncompPosts.loadPostings(oneQueryPostIds)) {
                    cerr << "Cannot load postings for the query '" << line
                         <<  "' after all postings are deleted. Perhaps, the memory budget is too small, or keepQtyPost is too large." << endl;
                    cerr << "Aborting!" << endl;
                    return -1;
                }
            }
        ...
        cout << "# Total lines: " << lineQty <<
             ...
             " skipped: " << skippedQty << " assigned randomly: " << randPickQty << endl;
